...
* 第三题难点在于如何在O(n)范围内找到；我一开始用ArrayList甚至尝试了LinkedList再排序，以为是O(nlgn)，但实际上还是TLE了，估计是在添加和删除元素时还是会遍历一遍，所以是O(n^2)；这时我明确目标，**抽象思维**，目的是找到一段子数组里的第几个，需要排序；这时注意到**数据范围(应该看完题目最先看的)**，就知道可以直接暴力，类似**计数排序&**
* 第四题难点在于我一直在想如何找到操作最短互相为1的一对数，实际上可以**仔细想想**，只需要暴力两轮即可
* 重要的是**数据范围**和**抽象思维(目标/逻辑)**，还有**享受思考过程**
